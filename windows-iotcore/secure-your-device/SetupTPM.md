...
---
# <a name="setting-up-tpm-on-suggested-platforms"></a>Einrichten von TPM auf Plattformen empfohlen

## <a name="setup-firmware-tpm-ftpm"></a>Einrichten der Firmware-TPM (fTPM)
Firmware-TPM (fTPM) erfordert spezielle Prozessor/SoC-Unterstützung und Bescheinigung fTPM zurzeit nicht auf Raspberry Pi2 implementiert ist.

1. Sie müssen MBM mit UEFI Version 0,80 oder höher verfügen.
2. Aktivieren Sie fTPM, indem Sie die folgenden UEFI-Einstellungen ändern:

        Device Manager -> System Setup -> Security Configuration -> PTT = <Enable>

3. Stellen Sie sicher, dass Sie nicht C:\Windows\System32\ACPITABL.dat für sTPM/dTPM (Auflösen der Konflikt/Löschen der Datei, wenn nicht benötigt).
4. Überprüfen, ob die richtige TPM-Version "enabled" – führen Sie die [TPM 2.0-Tool](https://github.com/ms-iot/security/tree/master/Urchin/T2T) auf dem Gerät Windows IoT Core.
...
5. Überprüfen Sie, ob fTPM funktionsfähigen: führen die [Urchin Komponententests](https://github.com/ms-iot/security/tree/master/Urchin/UrchinTest) auf dem Gerät Windows IoT Core.  
   Mehrere PASS-Tests (Beachten Sie, dass einige der Funktionen wird nicht von der fTPM, damit einige Fehlercodes erwartet werden) sollte angezeigt werden:
...
## <a name="setup-discrete-tpm-dtpm"></a>Setup diskretes TPM (dTPM)
Diese Anweisungen gelten für ein beliebiges dTPM-Modul auf MBM RPi2 oder RPi3 unterstützt.

1. Rufen Sie eine diskrete TPM-Modul, und fügen Sie ihn an die MBM/RPi2/RPi3.
2. (Gilt für MBM) Deaktivieren Sie fTPM, indem Sie die folgenden UEFI-Einstellungen ändern:

        Device Manager -> System Setup -> Security Configuration -> PTT = <Disable>

3. (Gilt für MBM) Aktivieren Sie dTPM, indem Sie die folgenden UEFI-Einstellungen ändern:

        Device Manager -> System Setup -> Security Configuration -> Discrete TPM = <Enable>

4. Basierend auf Ihrer Wahl diskrete TPM-Modul, identifizieren Sie die entsprechende ACPI-Tabelle [hier](https://github.com/ms-iot/security/tree/master/TPM-ACPITABL).
5. Kopieren Sie die ACPI-Tabelle in MBM/RPi2/RPi3 _C:\Windows\System32\ACPITABL.dat_.
6. Testsigning auf dem Gerät zu aktivieren:

        bcdedit /set {current} integrityservices disable
        bcdedit /set testsigning on

7. Sie können das Gerät neu starten.
8. Überprüfen, ob die richtige TPM-Version "enabled" – führen Sie die [TPM 2.0-Tool](https://github.com/ms-iot/security/tree/master/Urchin/T2T) auf dem Gerät Windows IoT Core.
...
9. Überprüfen Sie, ob dTPM funktionsfähigen: führen die [Urchin Komponententests](https://github.com/ms-iot/security/tree/master/Urchin/UrchinTest) auf dem Gerät Windows IoT Core.  
    Mehrere PASS-Tests (Beachten Sie, dass einige Funktionen nicht durch die dTPM unterstützt werden kann, damit einige Fehlercodes erwartet werden) sollte angezeigt werden:
...
## <a name="enable-and-verify-software-tpm-stpm"></a>Aktivieren Sie und überprüfen Sie die Software TPM (sTPM)  
Beachten Sie, dass **sTPM richtet sich an für die Entwicklung nur zu Evaluierungszwecken nutzen und bietet alle Vorteile der wirkliche Sicherheit keine**.

1. (Gilt für MBM) Deaktivieren Sie fTPM, indem Sie die folgenden UEFI-Einstellungen ändern:

        Device Manager -> System Setup -> Security Configuration -> PTT = <Disable>

2. (Gilt für MBM) Aktivieren Sie dTPM, indem Sie die folgenden UEFI-Einstellungen ändern:

        Device Manager -> System Setup -> Security Configuration -> Discrete TPM = <Enable>

3. Testsigning auf dem Gerät zu aktivieren:

        bcdedit /set {current} integrityservices disable
        bcdedit /set testsigning on

4. Kopieren Sie die ACPI-Tabelle aus [hier](https://github.com/ms-iot/security/tree/master/TPM-ACPITABL/fTPMSim) zu MBM/RPi2/RPi3 _C:\Windows\System32\ACPITABL.dat_.
5. Sie können das Gerät neu starten.
6. Überprüfen, ob die richtige TPM-Version "enabled" – führen Sie die [TPM 2.0-Tool](https://github.com/ms-iot/security/tree/master/Urchin/T2T) auf dem Gerät Windows IoT Core.
...
7. Überprüfen Sie, ob sTPM funktionsfähigen: führen die [Urchin Komponententests](https://github.com/ms-iot/security/tree/master/Urchin/UrchinTest) auf dem Gerät Windows IoT Core.  
   Mehrere PASS-Tests (Beachten Sie, dass einige der Funktionen wird nicht von der sTPM, damit einige Fehlercodes erwartet werden) sollte angezeigt werden:
